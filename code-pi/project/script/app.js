const clubId = "a8f25766-4981-4af3-b4d2-e33e7726fe43"
...
const choseSide = function(kleur){
    ...
    opslag_blauw.classList.remove("opslag-is-none")
    // else{
...
const endGame = function(winaar){
  ...
  if (winaar == "rood"){
    ...
    if(name1 != "" ^ name2 != ""){
      ...
      message += ` zijn gewonnen!`
    }
    else if(name1 == "" && name2 == ""){
      message += `Team 1 is gewonnen!`
    }
  }
  if (winaar == "blauw"){
    if(name3 != ""){
...
const nieuwGame = function(){
  ...
  gedaan.classList.add("opslag-is-none")
...
const changeName = function(kleur,naam1,naam2){
  ...
  if (kleur == "rood"){
    if(naam1 != ""){
      if(naam2 != ""){
        namenRood +=  `<p class="c-scoreboard-naam">${naam1} en&nbsp</p>`;
        name1 = naam1
      }
      else{
    ...
    if(naam1 != ""){
      if(naam2 != ""){
        namenBlauw +=  `<p class="c-scoreboard-naam">${naam1} en&nbsp</p>`;
        name3 = naam1
      }
      else{
...
const showSponsoren = function(jsonObject){
  ...
  var eigenSposoren = ``
  for (var sponsor of jsonObject){
    eigenSposoren += `<img class="c-sponsor" src="${sponsor.foto}" alt="${sponsor.naam}">`
  }
  sponsoren.innerHTML = eigenSposoren
}

const showLogo = function(jsonObject){
  const logo = document.querySelector(".js-logo")
  var eigenSposoren = ``
  eigenSposoren += `<img src="${jsonObject.logo}" alt="${jsonObject.naam}">`
  logo.innerHTML = eigenSposoren
}
...
let getAPI = async () => {
    ...
    handleData(endPoint, showSponsoren);
    handleData(endPoint2, showLogo);
